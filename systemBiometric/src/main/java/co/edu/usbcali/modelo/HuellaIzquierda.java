package co.edu.usbcali.modelo;
// Generated 14/05/2017 01:16:03 PM by Hibernate Tools 5.1.0.Alpha1

import java.util.Date;
import java.util.HashSet;
import java.util.Set;

/**
 * HuellaIzquierda generated by hbm2java
 */
public class HuellaIzquierda implements java.io.Serializable {

	private long hullId2;
	private byte[] huellaizquierda;
	private String usuCreador;
	private String usuModificador;
	private Date fechaCreador;
	private Date fechaModificador;
	private Set<UsuarioHuella> usuarioHuellas = new HashSet<UsuarioHuella>(0);

	public HuellaIzquierda() {
	}

	public HuellaIzquierda(long hullId2, String usuCreador, Date fechaCreador) {
		this.hullId2 = hullId2;
		this.usuCreador = usuCreador;
		this.fechaCreador = fechaCreador;
	}

	public HuellaIzquierda(long hullId2, byte[] huellaizquierda, String usuCreador, String usuModificador,
			Date fechaCreador, Date fechaModificador, Set<UsuarioHuella> usuarioHuellas) {
		this.hullId2 = hullId2;
		this.huellaizquierda = huellaizquierda;
		this.usuCreador = usuCreador;
		this.usuModificador = usuModificador;
		this.fechaCreador = fechaCreador;
		this.fechaModificador = fechaModificador;
		this.usuarioHuellas = usuarioHuellas;
	}

	public long getHullId2() {
		return this.hullId2;
	}

	public void setHullId2(long hullId2) {
		this.hullId2 = hullId2;
	}

	public byte[] getHuellaizquierda() {
		return this.huellaizquierda;
	}

	public void setHuellaizquierda(byte[] huellaizquierda) {
		this.huellaizquierda = huellaizquierda;
	}

	public String getUsuCreador() {
		return this.usuCreador;
	}

	public void setUsuCreador(String usuCreador) {
		this.usuCreador = usuCreador;
	}

	public String getUsuModificador() {
		return this.usuModificador;
	}

	public void setUsuModificador(String usuModificador) {
		this.usuModificador = usuModificador;
	}

	public Date getFechaCreador() {
		return this.fechaCreador;
	}

	public void setFechaCreador(Date fechaCreador) {
		this.fechaCreador = fechaCreador;
	}

	public Date getFechaModificador() {
		return this.fechaModificador;
	}

	public void setFechaModificador(Date fechaModificador) {
		this.fechaModificador = fechaModificador;
	}

	public Set<UsuarioHuella> getUsuarioHuellas() {
		return this.usuarioHuellas;
	}

	public void setUsuarioHuellas(Set<UsuarioHuella> usuarioHuellas) {
		this.usuarioHuellas = usuarioHuellas;
	}

}
